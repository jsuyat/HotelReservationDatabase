...
public class Room {
	...
	private void initialize() {
		...
		JLabel lblRNum = new JLabel("Room Type:");
		lblRNum.setBounds(6, 49, 78, 16);
		panel.add(lblRNum);

		JLabel lblParty = new JLabel("Capacity:");
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String roomTypeText = textField.getText();
				int capacityText = Integer.parseInt(textField_1.getText());
				...
				try{
					...
					database.insertData(roomTypeText, capacityText);
				}
		...
		JLabel lblAddHotelData = new JLabel("Update/Add new Room Data here:");
		...
		JLabel lblDeleteCustomerData = new JLabel("Delete Room Data Here:");
		lblDeleteCustomerData.setBounds(29, 17, 175, 16);
		panel_3.add(lblDeleteCustomerData);

		JLabel lblRnumDelete = new JLabel("Hotel Name:");
		...
		JLabel lblDelete = new JLabel("Branch ID:");
		lblDelete.setBounds(6, 84, 75, 16);
		panel_3.add(lblDelete);

		textField_9 = new JTextField();
		textField_9.setBounds(92, 79, 181, 26);
		...
		JButton btnSeeCustomerTable = new JButton("See Room Table");
		btnSeeCustomerTable.setBounds(30, 48, 149, 76);
		btnSeeCustomerTable.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				...
				String[] columns = new String[]{"Room Type", "Capacity"};
	...
	private Object[][] getRoomData(){
		try{
			RoomDatabase database = new RoomDatabase();
			RoomDatabase[] rooms = database.loadAll();
			Object[][] row = new Object[20][5];
			for(int i = 0 ; i < rooms.length; i++){
				row[i][0] = (Object) rooms[i].getRoomType();
				row[i][1] = (Object) rooms[i].getCapacity();
			}
			return row;
	  }
